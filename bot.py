...
MAX_POSITIONS = 3
STOP_LOSS_PCT = 0.02
SLEEP_SECONDS = 780  # 13 minutes — keeps total run safely under 20 min timeout
    ...
    try:
        requests.post(
            ...
            json={"symbol": symbol, "action": action, "quantity": quantity,
                  "price": float(price), "value": float(value), "rsi": float(rsi),
                  "ma20": float(ma20), "ma50": float(ma50), "risk_level": RISK_LEVEL},
            timeout=10)
        print(f"Trade logged: {action} {symbol}")
    except Exception as e:
...
def trigger_next_run():
    """
    Self-schedule: sleep 13 mins then trigger a new workflow run.
    Uses WORKFLOW_TOKEN (PAT) — not GITHUB_TOKEN — to avoid 403 restrictions.
    Only triggers if still within market hours after the sleep.
    """
    if not WORKFLOW_TOKEN:
        print("No WORKFLOW_TOKEN found — cannot self-schedule.")
        return

    print(f"Sleeping {SLEEP_SECONDS // 60} minutes before next run...")
    time.sleep(SLEEP_SECONDS)

    # Re-check market hours after sleep — don't trigger if market closed while sleeping
    now = datetime.now(timezone.utc)
    market_close = now.replace(hour=20, minute=0, second=0, microsecond=0)
    if now >= market_close or now.weekday() >= 5:
        print("Market closed during sleep — stopping chain for today.")
        return
...
def run_bot():
    ...
    if not clock.is_open:
        print("Market is closed — saving snapshot, stopping chain.")
        account = api.get_account()
        log_snapshot(float(account.portfolio_value), float(account.cash), 0, 0)
        send_telegram(
            "🕐 <b>Bot checked in</b>\nMarket is closed.\n"
            "Will resume automatically at next market open.")
        return  # Chain stops — no self-trigger
    ...
    for symbol, pos in held_symbols.items():
        ...
        if pnl_pct <= -STOP_LOSS_PCT:
            try:
                ...
                print(f"Stop-loss failed for {symbol}: {e}")
